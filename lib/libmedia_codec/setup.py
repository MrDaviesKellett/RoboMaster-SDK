...
if sys.version_info < (3, 14):
    sys.exit("libmedia_codec requires Python 3.14 or later")
if pybind11 is None:
    sys.exit("libmedia_codec requires pybind11 >= 3.0.3. Install it with 'python -m pip install pybind11>=3.0.3'.")
...
class CMakeBuild(build_ext):
    def run(self):
        ...
        if platform.system() == "Windows":
            cmake_version = tuple(int(part) for part in re.search(r'version\s*([\d.]+)', out.decode()).group(1).split('.'))
